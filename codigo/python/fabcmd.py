"""Script para manejar una capacidad Microsoft Fabric y para consultar el historial de actualizaciones de un modelo semántico publicado en el servicio de Power BI."""
...
def get_access_token(scope) -> str:
    """Se conecta a la API de Azure para pedir un token para los scopes indicados.
...
def get_fabric_capacities(access_token: str, subscription_id: str):
    ...
    # api_url = f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Fabric/capacities?api-version=2022-07-01-preview"
...
def callback(
    ...
    """Una herramienta de línea de comando para automatizar tareas de Fabric.

    Para ver la ayuda de un comando: fabcmd <comando> --help

    Para más detalles visite https://www.dataxbi.com/fabcmd/
    """
...
def refresh_history(
    group_id: Annotated[
        ...
        typer.Option(
            "--group",
            "-g",
            help="ID del a´rea de trabajo donde está el modelo semántico.",
            show_default=False,
        ),
    ],
    ...
    top: Annotated[
        ...
        typer.Option(
            "--top",
            "-t",
            help="Cantidad de elementos a mostrar.",
            show_default=True,
        ),
    ] = 1,
):
    """Muestra el historial de actualizaciones de un modelo semántico."""

    pbi_access_token = get_access_token(POWER_BI_SCOPE)
    refresh_history = get_refresh_history(
        pbi_access_token, group_id, dataset_id, top=top
    )
    ...
    for rh in refresh_history:
        table.add_row(rh["status"], rh["startTime"], rh["endTime"], rh["refreshType"])
